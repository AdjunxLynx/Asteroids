...
class Scoreboard():
    ...
    def get_integers(self):
        ...
        for i in range(0, len(all)):
            integers.append(re.sub('\D', '', all[i]))
        return integers
    ...
    def get_last_3_letters(self):
        ...
        letters = []
        for i in range(0, len(all)):
            temp = re.sub('[\W_]+', '', all[i])
            letters.append(list(temp[-3:]))
        return letters


    #create function to pair 2 lists together
    def score_lists(self):
        integers = self.get_integers()
        letters = self.get_last_3_letters()
        if integers == False or letters == False:
            return False
        paired = []
        if len(integers) == len(letters):
            for i in range(0, len(integers)):
                pair = []
                pair.append(integers[i])
                pair.append(letters[i])
                print(letters[i])
                paired.append(pair)
        print(paired)
        return paired
    ...
    def show_past_scores(self):
        ...
        if points == False:
            ...
            Game.display.blit(text, (Game.Dimensions / 2 - 75, Game.Dimensions / 2))
            return
        ...
        for i in range(0, len(self.score_lists())):
            text = Game.font.render(str(self.score_lists()[i]), True, pygame.Color("coral"))
            Game.display.blit(text, (Game.Dimensions / 2 - 75, Game.Dimensions / 2 + i * 25))
